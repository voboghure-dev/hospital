...
class MAc_debit_voucher_master extends CI_Model {
  ...
  function get_by_id($id) {
    $data = array();
    $this->db->where('id', $id);
    $q = $this->db->get('ac_debit_voucher_master');
    if ($q->num_rows() > 0) {
  ...
  function get_all() {
    ...
    if ($q->num_rows() > 0) {
      foreach ($q->result_array() as $row) {
        $details = MAc_debit_voucher_details::get_by_voucher_no($row['voucher_no']);
        $row['debit_amount'] = 0;
        $row['credit_amount'] = 0;
  ...
  function create() {
    ...
    $this->db->insert('ac_debit_voucher_master', $data);
  }
  ...
  function update() {
    ...
    $this->db->update('ac_debit_voucher_master', $data);
  }

  function delete($id) {
    $this->db->where('id', $id);
    $this->db->delete('ac_debit_voucher_master');
  }
