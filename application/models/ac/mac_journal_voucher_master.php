...
class MAc_journal_voucher_master extends CI_Model {
  ...
  function get_all() {
    ...
    if ($q->num_rows() > 0) {
      foreach ($q->result_array() as $row) {
        $details = MAc_journal_voucher_details::get_by_voucher_no($row['voucher_no']);
        $row['debit_amount'] = 0;
        $row['credit_amount'] = 0;
  ...
  function create() {
    ...
    $this->db->insert('ac_journal_voucher_master', $data);
  }
  ...
  function update() {
    ...
    $this->db->update('ac_journal_voucher_master', $data);
  }

  function delete($id) {
    $this->db->where('id', $id);
    $this->db->delete('ac_journal_voucher_master');
  }
