...
class MAc_debit_voucher_details extends CI_Model {
  ...
  function get_by_id($id) {
    $data = array();
    $this->db->where('id', $id);
    $q = $this->db->get('ac_debit_voucher_details');
    if ($q->num_rows() > 0) {
  ...
  function create() {
    ...
    $this->db->insert('ac_debit_voucher_details', $data);
  }
  ...
  function update() {
    ...
    $this->db->update('ac_debit_voucher_details', $data);
  }

  function delete($id) {
    $this->db->where('id', $id);
    $this->db->delete('ac_debit_voucher_details');
  }
