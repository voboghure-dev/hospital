...
class MAc_journal_voucher_details extends CI_Model {
  ...
  function get_by_id($id) {
    $data = array();
    $this->db->where('id', $id);
    $q = $this->db->get('ac_journal_voucher_details');
    if ($q->num_rows() > 0) {
  ...
  function get_voucher($chart, $voucher_no) {
    $data = array();
  ...
  function create() {
    ...
    $this->db->insert('ac_journal_voucher_details', $data);
  }
  ...
  function delete($id) {
    $this->db->where('id', $id);
    $this->db->delete('ac_journal_voucher_details');
  }
  
  function delete_by_voucher_no($voucher_no) {
    $this->db->where('voucher_no', $voucher_no);
    $this->db->delete('ac_journal_voucher_details');
  }
